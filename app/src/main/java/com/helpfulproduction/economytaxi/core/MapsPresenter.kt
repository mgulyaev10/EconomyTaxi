package com.helpfulproduction.economytaxi.core
...
import com.helpfulproduction.economytaxi.utils.extensions.toDefaultLocation
import com.helpfulproduction.economytaxi.utils.extensions.toMyLocation
import com.helpfulproduction.economytaxi.utils.extensions.toPosition
import com.helpfulproduction.economytaxi.utils.android.LocationHelper
import com.helpfulproduction.economytaxi.utils.android.PermissionHelper
import java.util.*
...
class MapsPresenter(
    ...
    override fun onCreateMapView(mapView: MapView?) {
        ...
        PermissionHelper.requestPermission(view.fragment(), Manifest.permission.ACCESS_FINE_LOCATION,
    ...
    override fun onPermissionWindowClick() {
        PermissionHelper.tryRequestPermission(view.fragment(), Manifest.permission.ACCESS_FINE_LOCATION,
            LOCATION_REQUEST_CODE
        )
    }

    override fun onDestinationSearchClick() {
    ...
    private fun resolveAddress() {
        try {
            ...
            val addressFull = addresses[0]
            val thoroughfare = addressFull.thoroughfare
            if (thoroughfare == null || thoroughfare == UNNAMED_ROAD) {
                view.onUnknownAddress()
            } else {
                val subThoroughfare = addressFull.subThoroughfare
                val address = if (subThoroughfare == null) {
                    thoroughfare
                } else {
                    "$thoroughfare, $subThoroughfare"
                }
                view.onAddressResolved(address)
